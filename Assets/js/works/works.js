...
document.querySelectorAll('.navbar a, .ra-card[href]:not([target="_blank"])').forEach((link) => {
  link.addEventListener("click", (event) => {
    ...
    // A file:// page has no authentication server of its own. Keep local
    // previews local by sending protected routes to the local Next.js server.
    // Deployed pages continue to use their current origin through link.href.
